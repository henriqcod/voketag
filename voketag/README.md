...
- **admin-service** (Python): Painel administrativo, God mode
...
- **Deploy production-ready:** `docs/DEPLOY_PRODUCTION.md` (staging, secrets, Cloud Run)
- **Admin Frontend:** `docs/ADMIN_FRONTEND.md` (URLs, variáveis, fluxo de login)
...
- **Prometheus**: `/metrics` em Scan, Factory, Admin, Blockchain
- **Grafana**: Dashboards provisionados (`infra/docker/grafana/`)
- **Flower**: Celery monitoring (`compose.monitoring.yml`)
- **Cloud Monitoring**: Métricas de CPU, memória, latência
- **Alertas**: P95 > 200ms, error rate > 1%

**Rodar stack de monitoramento:**
```bash
...
- **[Índice de Documentação](docs/DOCUMENTATION_INDEX.md)** - Portal central com todos os guias, auditorias e referências
